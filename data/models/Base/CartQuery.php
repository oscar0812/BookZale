 ...
 * @method     ChildCartQuery orderByBookId($order = Criteria::ASC) Order by the book_id column
 *
 * @method     ChildCartQuery groupByUserId() Group by the user_id column
 * @method     ChildCartQuery groupByBookId() Group by the book_id column
 *
 ...
 * @method     ChildCart findOneByBookId(int $book_id) Return the first ChildCart filtered by the book_id column *
 ...
 * @method     ChildCart requireOneByBookId(int $book_id) Return the first ChildCart filtered by the book_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildCart[]|ObjectCollection findByBookId(int $book_id) Return ChildCart objects filtered by the book_id column
...
abstract class CartQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT user_id, book_id FROM cart WHERE user_id = :p0 AND book_id = :p1';
        try {
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        $this->addUsingAlias(CartTableMap::COL_BOOK_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            $cton1 = $this->getNewCriterion(CartTableMap::COL_BOOK_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function filterByBookId($bookId = null, $comparison = null)
    {
        if (is_array($bookId)) {
            $useMinMax = false;
            if (isset($bookId['min'])) {
                $this->addUsingAlias(CartTableMap::COL_BOOK_ID, $bookId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($bookId['max'])) {
                $this->addUsingAlias(CartTableMap::COL_BOOK_ID, $bookId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CartTableMap::COL_BOOK_ID, $bookId, $comparison);
    }
    ...
    public function filterByBook($book, $comparison = null)
    {
        if ($book instanceof \Book) {
            return $this
                ->addUsingAlias(CartTableMap::COL_BOOK_ID, $book->getId(), $comparison);
    ...
    public function prune($cart = null)
    {
        if ($cart) {
            ...
            $this->addCond('pruneCond1', $this->getAliasedColName(CartTableMap::COL_BOOK_ID), $cart->getBookId(), Criteria::NOT_EQUAL);
