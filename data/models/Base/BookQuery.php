 ...
 *
 * @method     ChildBookQuery joinWithUser($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the User relation
 *
 * @method     ChildBookQuery leftJoinWithUser() Adds a LEFT JOIN clause and with to the query using the User relation
...
abstract class BookQuery extends ModelCriteria
{
    ...
    public function filterByPostedBy($postedBy = null, $comparison = null)
    {
        if (is_array($postedBy)) {
            ...
            if (isset($postedBy['min'])) {
                $this->addUsingAlias(BookTableMap::COL_POSTED_BY, $postedBy['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($postedBy['max'])) {
                $this->addUsingAlias(BookTableMap::COL_POSTED_BY, $postedBy['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(BookTableMap::COL_POSTED_BY, $postedBy, $comparison);
    }
     ...
     * Example usage:
     * <code>
     * $query->filterBySold(true); // WHERE sold = true
     * $query->filterBySold('yes'); // WHERE sold = true
     * </code>
     *
     * @param     boolean|string $sold The value to use as filter.
    ...
    {
        if (is_string($sold)) {
            $sold = in_array(strtolower($sold), array('false', 'off', '-', 'no', 'n', '0', '')) ? false : true;
        }

        return $this->addUsingAlias(BookTableMap::COL_SOLD, $sold, $comparison);
    }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof \User) {
            return $this
                ->addUsingAlias(BookTableMap::COL_POSTED_BY, $user->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(BookTableMap::COL_POSTED_BY, $user->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
     ...
     * @return ChildBookQuery The current query, for fluid interface
     */
    public function filterBycartUser($user, $comparison = Criteria::EQUAL)
    {
        return $this
            ->useCartQuery()
            ->filterBycartUser($user, $comparison)
            ->endUse();
    }
     ...
     * @return ChildBookQuery The current query, for fluid interface
     */
    public function filterBycurrentUser($user, $comparison = Criteria::EQUAL)
    {
        return $this
            ->useWishlistQuery()
            ->filterBycurrentUser($user, $comparison)
            ->endUse();
    }
