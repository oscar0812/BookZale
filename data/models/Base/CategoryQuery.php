 ...
 *
 * @method     ChildCategoryQuery leftJoinBook($relationAlias = null) Adds a LEFT JOIN clause to the query using the Book relation
 * @method     ChildCategoryQuery rightJoinBook($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Book relation
 * @method     ChildCategoryQuery innerJoinBook($relationAlias = null) Adds a INNER JOIN clause to the query using the Book relation
 *
 * @method     ChildCategoryQuery joinWithBook($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Book relation
 *
 * @method     ChildCategoryQuery leftJoinWithBook() Adds a LEFT JOIN clause and with to the query using the Book relation
 * @method     ChildCategoryQuery rightJoinWithBook() Adds a RIGHT JOIN clause and with to the query using the Book relation
 * @method     ChildCategoryQuery innerJoinWithBook() Adds a INNER JOIN clause and with to the query using the Book relation
 *
 ...
 * @method     ChildCategory requireOneByParent(int $parent) Return the first ChildCategory filtered by the parent column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildCategory[]|ObjectCollection findByParent(int $parent) Return ChildCategory objects filtered by the parent column
...
abstract class CategoryQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, parent, name, description FROM category WHERE id = :p0';
        try {
    ...
    public function filterByParent($parent = null, $comparison = null)
    {
        if (is_array($parent)) {
            $useMinMax = false;
            if (isset($parent['min'])) {
                $this->addUsingAlias(CategoryTableMap::COL_PARENT, $parent['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($parent['max'])) {
                $this->addUsingAlias(CategoryTableMap::COL_PARENT, $parent['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(CategoryTableMap::COL_PARENT, $parent, $comparison);
    }
    ...
    public function filterByBook($book, $comparison = null)
    {
        if ($book instanceof \Book) {
            return $this
                ->addUsingAlias(CategoryTableMap::COL_ID, $book->getCategoryId(), $comparison);
